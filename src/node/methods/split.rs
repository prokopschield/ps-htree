...
impl<T> HtreeNode<T> {
    /// Splits the tree at the partition point of the predicate.
    ///
    /// Assumes the leaves are sorted such that the predicate is monotonic:
    /// all elements for which the predicate returns `true` come first, followed
    /// by all elements for which it returns `false` — matching the contract of
    ...
    ///
    /// Returns `(left, right)` where `left` contains all leaves for which the
    /// predicate returns `true` and `right` contains the rest.
    ///
    /// # Arguments
    ///
    /// * `predicate` - A monotonic predicate: `true` for left elements, `false`
    ///   for right.
        ...
        F: Fn(&Self) -> bool,
    {
        ...
        let (mut left, mut right) = loop {
            ...
            if split_idx == 0 {
                // Predicate is false for the first child, so by monotonicity all leaves
                // belong to the right half.
...
mod tests {
    ...
    #[test]
    fn leaf_matches_goes_to_left() -> Result<(), Box<dyn std::error::Error>> {
        let store = InMemoryStore::default();
        let key = UUID::gen_v4();
    ...
    #[test]
    fn leaf_does_not_match_goes_to_right() -> Result<(), Box<dyn std::error::Error>> {
        let store = InMemoryStore::default();
        let key = UUID::gen_v4();
    ...
    #[test]
    fn none_match_returns_none_some() -> Result<(), Box<dyn std::error::Error>> {
        let store = InMemoryStore::default();
        let keys = gen_keys(5);
        let tree = make_tree(&keys, &store)?;

        // Predicate is false for all keys → all go right
    ...
    #[test]
    fn all_match_returns_some_none() -> Result<(), Box<dyn std::error::Error>> {
        let store = InMemoryStore::default();
        let all = gen_keys(6);
        let tree_keys = &all[..5];
    ...
    #[test]
    fn predicate_lt_nil_puts_everything_right() -> Result<(), Box<dyn std::error::Error>> {
        let store = InMemoryStore::default();
        let keys = gen_keys(5);
        let tree = make_tree(&keys, &store)?;
    ...
    #[test]
    fn left_keys_match_predicate() -> Result<(), Box<dyn std::error::Error>> {
        let store = InMemoryStore::default();
    ...
    #[test]
    fn right_keys_do_not_match_predicate() -> Result<(), Box<dyn std::error::Error>> {
        let store = InMemoryStore::default();
        let keys = gen_keys(10);
        let tree = make_tree(&keys, &store)?;
        let threshold = keys[5];
